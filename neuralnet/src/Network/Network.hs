...
import           GHC.TypeLits
import           Prelude           hiding (foldl, head, length, map, mapM, sum,
                                    tail, zipWith, (!!), (++))
...
instance Show (Network i l o) where
    show NilNetwork            = "End of network"
    show (neurons :~~ netTail) = "Layer:\n" +++ snd (foldl (\(i, p) n -> (i+1, p +++ "Neuron #" +++ show i +++ ":\n" +++ show n +++ "\n")) (1, "") neurons) +++ "\n" +++ show netTail
      where (+++) = (P.++)
